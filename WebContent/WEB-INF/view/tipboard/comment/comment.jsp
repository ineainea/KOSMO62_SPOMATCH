<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="kosmo62.spomatch.comment.vo.SpoCommentVO" %>
...
<%
		...
		if(nCnt > 0){
			for(int i = 0; i < nCnt; i++){
				...
				String smb_id = param.getSmb_id();
				System.out.println("stb_no >>> " + stb_no);
				System.out.println("smb_no >>> " + smb_no);
				System.out.println("smb_id >>> " + smb_id);
%>
<script type="text/javascript">
	...
	function insertcommentFunc(){
		...
		console.log("stb_no >>> " + stb_no +", smb_no >>> " + smb_no);
		

		var scm_content = $("#scm_content").val();
		
		var urls = "insertcomment.spo";
	...
	function addNewcomment(listcomment){
		...
		for(var i = 0; i<listcomment.length; i++){
			...
			// 작성자명
			var smb_id_span = $("<span>");
			smb_id_span.addClass("bold");
			smb_id_span.addClass("marL5");
			smb_id_span.html(smb_id + "&nbsp;&nbsp;&nbsp;");
			...
			*/
			// 조립하기
			info_p.append(no_span).append(smb_id_span).append(scm_insertdate_span)
